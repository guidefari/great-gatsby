...
const mixCard = ({data}) => {
    return (
        <div className="flex px-4 mb-4 ">
            {data.map(item => (
                <div key={item.node.id} className="flex flex-col w-full h-full p-8 mx-6 rounded-lg bg-custom-gray-seconday card">
                <div className="flex items-center h-24 mb-3">
                  ...
                  <div className="flex justify-between w-full">
                    ...
                    <h3 className="text-sm text-right text-custom-olive-300">Mixed by <br/>{item.node.frontmatter.DJ} </h3>
                  </div>
                </div>
                <div className="flex flex-col justify-between flex-grow">
                  ...
                  <a href={item.node.frontmatter.external} target="_blank" rel="noreferrer" className="inline-flex items-center w-auto mt-3 link-colors link-transition">Stream Mix
