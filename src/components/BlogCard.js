        ...
        <div className="flex flex-col justify-start h-full p-6 bg-custom-gray-seconday min-h-10 max-h-xs">
          <div>
            {category.map((cat) => (
              <span
                key={cat}
                className="mb-1 mr-2 text-xs font-medium tracking-widest title-font"
              >
                {cat}
              </span>
            ))}
            <h1 className="mb-3 overflow-y-auto text-xl font-semibold text-custom-olive-500 title-font">
              {title}
            </h1>
